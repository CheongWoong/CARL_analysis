...
class TD3(nn.Module):
    ...
    def learn(self, data, global_step):
        ...
        if self.qf_context_encoder is not None:
            _, _, qf_context_loss = self.qf_context_encoder(**data.observations, action=data.actions, next_obs=data.next_observations["obs"])
        ...
        if global_step % self.args.policy_frequency == 0:
            ...
            if self.actor_context_encoder is not None:
                _, _, actor_context_loss = self.actor_context_encoder(**data.observations, action=data.actions, next_obs=data.next_observations["obs"])
