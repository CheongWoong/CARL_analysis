...
class QNetwork(nn.Module):
    def __init__(self, env, args, context_encoder=None, gt_context_encoder=None):
        ...
        context_hidden_dim = args.context_hidden_dim*(context_encoder is not None) + gt_context_dim*(gt_context_encoder is not None)
    ...
    def forward(self, obs, action, context=None, history=None):
        ...
        if self.context_encoder is not None:
            context_hidden = self.context_encoder.get_context(history)
            x.append(context_hidden)
        if self.gt_context_encoder is not None:
            gt_context_hidden = self.gt_context_encoder(context)
...
class TD3(nn.Module):
    def __init__(self, envs, args, device):
        ...
        if args.len_history > 0:
            self.context_encoder = ContextEncoder(envs, args)
        ...
        self.actor = Actor(envs, args, self.context_encoder, self.gt_context_encoder).to(device)
        self.qf1 = QNetwork(envs, args, self.context_encoder, self.gt_context_encoder).to(device)
        self.qf2 = QNetwork(envs, args, self.context_encoder, self.gt_context_encoder).to(device)
        self.qf1_target = QNetwork(envs, args, self.context_encoder, self.gt_context_encoder).to(device)
        self.qf2_target = QNetwork(envs, args, self.context_encoder, self.gt_context_encoder).to(device)
        self.target_actor = Actor(envs, args, self.context_encoder, self.gt_context_encoder).to(device)
    ...
    def learn(self, data, global_step):
        ...
        # optimize the context encoder
        context_loss = None
        if self.args.context_objective in ["osi", "dm"]:
            _, context_loss = self.context_encoder(**data.observations, action=data.actions, next_obs=data.next_observations["obs"])
            self.context_optimizer.zero_grad()
            context_loss.backward()
            self.context_optimizer.step()

        if global_step % self.args.policy_frequency == 0:
            actor_loss = -self.qf1(**data.observations, action=self.actor(**data.observations)).mean()
